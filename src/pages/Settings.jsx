...
export default function SettingsPage() {
  const { theme, setTheme } = useContext(ThemeContext); // ⬅️ Use global theme
...
const [showProfileBox, setShowProfileBox] = useState(true);
  ...
  return (
    <div className={`min-h-screen p-8 font-sans ${theme === 'light' ? 'bg-[#FAFAFC] text-[#1F1F2E]' : 'bg-[#1F1F2E] text-[#FAFAFC]'}`}>
          ...
          <div className="p-4 border border-[#E0E0F0] rounded-lg bg-[#a0d29d]">
            ...
            <label className="flex items-center cursor-pointer">
              ...
              <div className="relative w-11 h-6">
                ...
                <div
                  className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full shadow transition-transform duration-200 ${theme === 'light' ? 'translate-x-5' : ''}`}
                ></div>
              </div>
  ...
  <div className={`mt-3 p-3 bg-[#E4F7E7] rounded border ${showProfileBox ? 'border-[#0B7557]' : 'border-[#1F1F2E]'} text-sm text-gray-700 ${showProfileBox !== null ? 'block' : 'hidden'}`}>
    {showProfileBox ? (
      ...
      <section className="mb-8">
        ...
        <div className="p-6 border border-[#E0E0F0] rounded-lg bg-[#a0d29d] text-center text-sm text-[#1F1F2E]">
